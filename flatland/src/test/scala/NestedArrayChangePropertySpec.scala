...
object Helpers {
  ...
  implicit class RichShrink[T](val shrink: Shrink[T]) extends AnyVal {
    def append(that: Shrink[T]) = Shrink { (t: T) =>
      this.shrink.shrink(t) append that.shrink(t)
...
class NestedArrayChangePropertySpec extends AnyFreeSpec with Checkers {
  ...
  val nestedArraysWithChanges = for {
    ...
    addElem <- if (nested.length + addIdx == 0)
                 ...
                 Gen
                   .containerOf[Vector, (Int, Int)](for {
    ...
    delElem <- for {
                 ...
                 delElem              <- Gen
                                           .sequence[Vector[Vector[(Int, Int)]], Vector[(Int, Int)]](
                                             sliceSubsetWithIndex.map { case (slice, idx) =>
  ...
  assert(
    removeInnerElemFromNested(
      NestedWithChanges(
        Vector(Vector(1, 7, 9), Vector(2, 5, 3, 4)),
        1,
        Vector(0 -> 5, 2 -> 4),
        Vector(0 -> 0, 0 -> 2, 1 -> 3),
      ),
      ...
      NestedWithChanges(
        Vector(Vector(1, 9), Vector(2, 5, 3, 4)),
        1,
        Vector(0 -> 5, 2 -> 4),
        Vector(0 -> 0, 0 -> 1, 1 -> 3),
      ),
  ...
  check(
    forAll(nestedArraysWithChanges) { case NestedWithChanges(nestedVector, addIdx, addElemVector, delElemVector) =>
      ...
      val changedArray = {
        ...
        addElem.foreach { case (idx, elem) =>
          result(idx) :+= elem
        }
        delElem.reverseIterator.foreach { case (idx, pos) =>
          result(idx) = (result(idx).take(pos) ++ result(idx).drop(pos + 1))
        }
        result
      }
