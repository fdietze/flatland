...
trait ArraySliceInt extends mutable.IndexedSeq[Int] {
  def length: Int

  @inline override def apply(idx: Int): Int

  @inline override def isEmpty = length == 0
  @inline override def slice(from: Int, until: Int): mutable.IndexedSeq[Int]
...
@inline final class ArraySliceIntValues(array: Array[Int], var start: Int, var length: Int) extends ArraySliceInt {
  ...
  @inline def update(idx: Int, elem: Int): Unit                    = array.update(start + idx, elem)
}

@inline final class ArraySliceIntMapped(array: Array[Int], var start: Int, var length: Int, mapped: Int => Int)
